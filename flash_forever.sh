#!/bin/bash
echo "[MONITOR] Escuchando activamente en /dev/ttyACM0 para flashear..."
...
while true; do
    if [ -e "/dev/ttyACM0" ]; then
        /home/diego/.platformio/penv/bin/esptool --chip esp32c3 --port /dev/ttyACM0 --baud 460800 \
            ...
            0x10000 .pio/build/esp32c3_super_mini/firmware.bin 2>&1
        
        if [ $? -eq 0 ]; then
            echo "=================================================="
            echo "🎉 ¡EXITO! FIRMWARE FLASHEADO AL 100% EN TU PLACA"
            echo "=================================================="
            exit 0
        fi
    fi
